...
describe('SchemaWizard', () => {
  ...
  it('populates the formModel with default values', async () => {
    const val = ref({})
    const wizardSchemaWithDefault = [...wizardSchema]
    wizardSchemaWithDefault[0].firstName.default = 'John'
    wizardSchemaWithDefault[0].lastName.default = 'Wick'
    wizardSchemaWithDefault[1].email.default = 'john.wick@gmail.com'
    mount(SchemaWrapperFactory(wizardSchemaWithDefault, {
      step: 0
    }, val))
    expect(val.value).toEqual(expect.objectContaining({
      firstName: 'John',
      lastName: 'Wick',
